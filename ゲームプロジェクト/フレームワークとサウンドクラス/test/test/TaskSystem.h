...
public:
	...
	Task() : m_priority(0), m_isActive(false), m_id(CAST::NONE), m_guard(false),m_pos(VGet(0.0f,0.0f,0.0f)){}
	virtual ~Task() {}
	virtual void Update() {}			//タスクの更新処理
	virtual void Draw() const {}		//タスクの描画
	virtual void Deth() {}		//デストラクタでする処理を実行する関数(MV1DeleteModelなど)タスクのデストラクタを働かないため
};
