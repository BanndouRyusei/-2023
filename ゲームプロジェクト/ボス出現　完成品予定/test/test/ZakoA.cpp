...
const int SHOT_WAIT = 180;     //エネミーのSHOT間隔
const float ZAKOA_SHADOW_Y = WINDOW_H - ZAKOA_H - 50.0f - OBJECT_Z_MAX;
...
CZakoA::CZakoA(float _ex, float _ey, float _ez, float _priortiy):CCharaBase(_priortiy,CAST::ZAKO_A) {
	...
	m_ShadowImage = LoadGraph("Data\\img\\ZakoAShadow.png");
	if (m_ShadowImage == -1)assert(false);
	m_Shadow.SetShadow(m_ShadowImage, m_Pos, m_Turn, ZAKOA_SHADOW_Y);
}
...
void CZakoA::Update() {
	...
	case e_ENEMYSTATE::ATTACK:
			if (m_ShotFlag && m_Frame == ZAKOA_ATTACKFLAME) {
				if (m_Turn) {
					m_ShotPosX = ZAKOA_W;
				}
				else {
...
void CZakoA::CheckPos() {
	float right = m_Pos.x + ZAKOA_W;
