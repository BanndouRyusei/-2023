...
void CTitleMenu::Update(void) {
	...
	if (!m_IsDecide) {			//まだメニューを決めていなかったら
		//選ばれているメニュー文字を点滅させる
		m_Alpha += TITLE_MENU_ALPHA_STEP * m_AlphaTurn;
		if (m_Alpha > ALPHA_MAX || m_Alpha < TITLE_MENU_ALPHA_MIN) {
			m_Alpha = ALPHA_MAX * m_AlphaTurn;			//値の初期化(ALPHA_MAXより大きい場合はALPHA_MAX)
			if (m_Alpha < TITLE_MENU_ALPHA_MIN) {			//TITLE_MENU_ALPHA_MINより小さい場合はTITLE_MENU_ALPHA_MIN
				m_Alpha = TITLE_MENU_ALPHA_MIN;
			}
	...
	if (!m_IsDecide&&IsKeyTrigger(key, PAD_INPUT_UP)) {
		CControlSound::GetInstance()->CreateSE(e_SE::CHANGE);		//メニュー移動音再生
		m_Alpha = ALPHA_MAX;
		m_AlphaTurn = -1;
		m_Item--;
	}
