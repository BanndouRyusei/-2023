...
CControlSound::CControlSound(float _priority) :CTask(_priority, CAST::CONTROLSOUND) {
	IsGuardFlg(true);
	ms_Instance = this;
	//音ファイル読込
...
CSE::CSE(const int _se, float _priority, int _playtype) :CTask(_priority, CAST::SE) {
	m_Se = _se;
...
CBGM::CBGM(const float _priority, int *_bgm) :CTask(_priority, CAST::BGM) {
