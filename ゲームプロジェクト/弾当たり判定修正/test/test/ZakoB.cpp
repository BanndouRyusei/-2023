...
CEnemyZakoB::CEnemyZakoB(float _ex, float _ez, float _speed, float _priortiy) :CTask(_priortiy, CAST::ZAKO_A) {
	m_DrawTask.Init(this, std::bind(&CEnemyZakoB::Draw, this), OBJECT_DRAWPRIORITY_MIN);
...
void CEnemyZakoB::Draw()const {
	DrawObject(m_Pos, m_AnimImage[(int)m_Phase][0], false, 0.0f);
}

bool CEnemyZakoB::HitCheck(const VECTOR &_pos, float _zwidth, int _ghandle) {
