...
float ChangePriority(const float* _z) {
	return *_z / OBJECT_Z_MAX + OBJECT_DRAWPRIORITY_MIN;
}

//ベルトスクロール用オブジェクト描画関数
void DrawObject(const VECTOR& _pos, const int _Grhandle, const bool _isturn) {
	//画像サイズを取得
	float SizeX, SizeY;
	GetGraphSizeF(_Grhandle, &SizeX, &SizeY);
	DrawRotaGraph2F(_pos.x + SizeX / 2, WINDOW_H - _pos.y - _pos.z - SizeY / 2.0f, SizeX / 2.0f, SizeY / 2.0f, 1.0, 0.0f, _Grhandle, _isturn);
}